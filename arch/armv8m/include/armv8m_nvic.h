#ifndef __ARMV8M__NVIC__H__
#define __ARMV8M__NVIC__H__

#include <stdint.h>
#include <stdbool.h>

#define ARMV8M_NVIC_REGBASE_ISER (ARMV8M_SCS + 0x100) /* Interrupt Set-Enable registers (15) */
#define ARMV8M_NVIC_REGBASE_ICER (ARMV8M_SCS + 0x180) /* Interrupt Clear-Enable registers (15) */
#define ARMV8M_NVIC_REGBASE_ISPR (ARMV8M_SCS + 0x200) /* Interrupt Set-Pending registers (15) */
#define ARMV8M_NVIC_REGBASE_ICPR (ARMV8M_SCS + 0x280) /* Interrupt Clear-Pending registers (15) */
#define ARMV8M_NVIC_REGBASE_IABR (ARMV8M_SCS + 0x300) /* Interrupt Active registers (15) */
#define ARMV8M_NVIC_REGBASE_IPR  (ARMV8M_SCS + 0x400) /* Interrupt Priority registers (123) */

/* These are the registers saved during exception entry */
struct armv8m_excontext_t
{
 ...
 *    - casted to struct armv8m_excontext_t to get values of saved registers
 *    - changed to support context switching
 * - an argument word that was saved during irq attach
 */
typedef void (*armv8m_irqhandler_f)(uint32_t irqno, void **context, void *arg);
