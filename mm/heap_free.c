...
int heap_free(struct heap_s *heap, handle_t handle)
{
  ...
  prev = NULL;
  for(cur = heap->ffree; cur != NULL; prev=cur, cur = cur->next)
    {
  ...
  if(prev) info("stat: prev: %p size %u next %p\n", prev, prev->size, prev->next);
  info("stat: this: %p size %u next %p\n", fb  ,   fb->size,   fb->next);
  if(cur) info("stat: cur : %p size %u next %p\n", cur ,  cur->size,  cur->next);
#if 0
